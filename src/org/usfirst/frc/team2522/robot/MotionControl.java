...
public class MotionControl 
{
	...
	public static MotionPosition getExpectedRotationPosition(double t, double angle, double vi, double maxVel, double maxAccl)
	{
		/**
		 * Maximum deceleration value.
		 */
		...
		if (Math.abs(angle) >= Math.abs(accl_d + break_d))
		{
			cruise_d = angle - (accl_d + break_d);
			cruise_t = getTimeToDistance(cruise_d, accl_vf, 0.0);
		}
		else
		{
			accl_d =  (maxAccl * angle) / (maxAccl - maxBreak);
			accl_t = getTimeToDistance(accl_d, accl_vi, maxAccl);
			accl_vf = getVelocity(accl_t, accl_vi, maxAccl);
			
			break_vi = accl_vf;
			break_d = angle - accl_d;
			break_t = getTimeToVelocity(break_vf, break_vi, maxBreak);
		}		
		...
		// Calculate the expected angle and velocity the robot should have achieved by the time specified.
		//
		double expectedBearing = 0.0;
		double expectedVelocity = 0.0;
		double expectedAcceleration = 0.0;
		...
		{
			expectedVelocity = getVelocity(t, accl_vi, maxAccl);
			expectedBearing = getDistance(t, accl_vi, maxAccl);
			expectedAcceleration = maxAccl;
		}
		else if (t <= (accl_t + cruise_t))
		{
			expectedAcceleration = 0.0;
			expectedVelocity = accl_vf;
			expectedBearing = accl_d + getDistance(t - accl_t, accl_vf, 0.0);
		}
		else if (t <= (accl_t + cruise_t + break_t))
		{
			expectedVelocity = getVelocity(t - (accl_t + cruise_t), break_vi, maxBreak);
			expectedBearing = accl_d + cruise_d + getDistance(t - (accl_t + cruise_t), break_vi, maxBreak);
			expectedAcceleration = maxBreak;
		}
		else
		{
			expectedVelocity = 0.0;
			expectedBearing = angle;
			expectedAcceleration = 0.0;
		}
